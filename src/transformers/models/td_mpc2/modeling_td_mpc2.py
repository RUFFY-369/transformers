...
# Copied from transformers.models.gpt2.modeling_gpt2.GPT2Attention with GPT2->TdMpc2GPT2
class TdMpc2GPT2Attention(nn.Module):
        ...
        if encoder_hidden_states is not None:
            if not hasattr(self, "q_attn"):
                raise ValueError(
                    ...
                    "Please make sure to instantiate class with `TdMpc2GPT2Attention(..., is_cross_attention=True)`."
                )
...
# Copied from transformers.models.gpt2.modeling_gpt2.GPT2Block with GPT2->TdMpc2GPT2
class TdMpc2GPT2Block(nn.Module):
    # Ignore copy
    def __init__(self, config, layer_idx=None):
        ...
        self.attn = TdMpc2GPT2Attention(config, layer_idx=layer_idx)
        self.ln_2 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)

        if config.add_cross_attention:
            self.crossattention = TdMpc2GPT2Attention(
                config, is_cross_attention=True, layer_idx=layer_idx
            )
            self.ln_cross_attn = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)

        self.mlp = TdMpc2GPT2MLP(inner_dim, config)
...
# Copied from transformers.models.decision_transformer.modeling_decision_transformer.DecisionTransformerGPT2PreTrainedModel with DecisionTransformer->TdMpc2
class TdMpc2GPT2PreTrainedModel(PreTrainedModel):
    """
...
# Copied from transformers.models.decision_transformer.modeling_decision_transformer.DecisionTransformerGPT2Model with DecisionTransformer->TdMpc2
class TdMpc2GPT2Model(TdMpc2GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList(
            [TdMpc2GPT2Block(config, layer_idx=i) for i in range(config.num_hidden_layers)]
        )
...
class TdMpc2Model(TdMpc2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.encoder = TdMpc2GPT2Model(config)
